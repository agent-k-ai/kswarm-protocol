...
#
# Exit codes: 0 the log reports <expected> passing tests and no failures,
...
case "${expected}" in
  ''|*[!0-9]*) echo "assert-test-count: expected count must be a number, got '${expected}'" >&2; exit 2 ;;
esac
if [ ! -f "${log}" ]; then
...
if [ "${passed}" -ne "${expected}" ]; then
  echo "assert-test-count: ${log} reports ${passed} passing tests, expected ${expected}" >&2
